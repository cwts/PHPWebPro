...
class ImagesController extends RestController
{
    ...
    public function add()
    {
        ...
        if (!IS_POST) {
        	...
        	$model=M('modulecategory');
        	$modulecategory=$model->select();
        	$this->assign("category",$modulecategory);
        	$this->time=time();
            $this->display();
        }
        if (IS_POST) {
...
//             $model->user_id = 1; 
            if (!$model->create()) {
                ...
                if ($model->add()) {
                    $this->success("添加成功", U('Images/index'));
                } else {
                    $this->error("添加失败");
                }
            }
...
//     			$img->save($tumb_url);//保存缩略图
    ...
    {
    	if (!IS_POST) {
            $model = M('images')->where('id='.$id)->find();
            $category=M('modulecategory');
            $modulecategory=$category->select();
            $this->assign("category",$modulecategory);
            ...
            if (IS_POST) {
            	...
            	if (!$model->create()) {
            		...
            		if ($model->save()) {
            			$this->success("更新成功", U('images/index'));
            		} else {
            			$this->error("更新失败");
            		}
            	}
    ...
    public function delete($id)
    {
        ...
        $url=$data[0][url];    
        $len=strlen(__ROOT__);
        $img_url=substr($url, $len);
       // echo "$img_url";
       //同时在数据库和文件夹下删除
        if (@unlink($img_url)&&$model->where("id=".$id)->delete()) {    
            $this->success("删除成功", U('Images/index'));     
        }else{
        	$this->error("删除失败");
        }             
    }
