 ...
 * 社工之家管理
 */
class WorkhouseController extends RestController
{
    /**
     * 社工列表
    ...
    public function index($key="")
    {
    	...
    	{
        if($key == ""){
            $model = D('CommunitymembersView'); 
        }else{
            $where['Communitymembers.name'] = array('like',"%$key%");
            $where['member.username'] = array('like',"%$key%");
          //  $where['category.title'] = array('like',"%$key%");
            $where['_logic'] = 'or';
            $model = D('CommunitymembersView')->where($where); 
        } 
        
        $count  = $model->where($where)->count();// 查询满足要求的总记录数
        $Page = new \Extend\Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        $show = $Page->show();// 分页显示输出
        $post = $model->limit($Page->firstRow.','.$Page->listRows)->where($where)->order('communitymembers.id DESC')->select();
...
//          	$data ['content'] = I ( $post.duty, '' );
//          	$data ['major'] = I ( $post.major, '' );
//         	$data [qualification] = I ( $post.qualification, '' );
//         	$data [year] = I ( $post.year, '' );
//         	$data [photo] = I ( $post.photo, '' );
//         	$data [photoname] = I ( $post.photoname, '' );
//         	$data [user_id] =session('adminId');
//         	echo dump($data);
//             $employee->data($data)->add();
        	        	$model = D("Communitymembers");
        	        	if (!$model->create()) {
        	        		$this->error($model->getError());
        	        	}else{
        	        		if ($model->add()) {
        	        			$this->success("更新成功", U('Workhouse/index'));
        	        		} else {
        	        			$this->error("更新失败");
        	        		}
        	        	}
 ...
 public function changeName(){
    ...
    {
        $model = M('Communitymembers');
        $result = $model->where("id=".$id)->delete();
        if($result){
            $this->success("删除成功", U('Workhouse/index'));
        }else{
            $this->error("删除失败");
        }
    }
