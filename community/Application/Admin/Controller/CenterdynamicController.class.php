 ...
 * 文章管理
 */
class CenterdynamicController extends RestController
{
    /**
     * 新闻列表
    ...
    public function index($key="")
    {
    	...
    	{
        if($key == ""){
            $model = D('CentraldynamicsView'); 
        }else{
            $where['centraldynamics.title'] = array('like',"%$key%");
            $where['member.username'] = array('like',"%$key%");
          //  $where['category.title'] = array('like',"%$key%");
            $where['_logic'] = 'or';
            $model = D('CentraldynamicsView')->where($where); 
        } 
...
//             $model->user_id = 1; 
            if (!$model->create()) {
                ...
                if ($model->add()) {
                    $this->success("添加成功", U('Centerdynamic/index'));
                } else {
                    $this->error("添加失败");
                }
            }
    ...
    public function update($id)
    {
        ...
        if (IS_POST) {
            ...
            if (!$model->create()) {
                ...
                if ($model->save()) {
                    $this->success("更新成功", U('Centerdynamic/index'));
                } else {
                    $this->error("更新失败");
                }        
            }
    ...
    {
        $model = M('Centraldynamics');
        $result = $model->where("id=".$id)->delete();
        if($result){
            $this->success("删除成功", U('Centerdynamic/index'));
        }else{
            $this->error("删除失败");
        }
    }
