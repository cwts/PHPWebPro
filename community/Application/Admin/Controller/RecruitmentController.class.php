 ...
 * 招聘信息管理
 */
class RecruitmentController extends RestController
{
    /**
     * 招聘信息列表
    ...
    public function index($key="")
    {
    	...
    	{
        if($key == ""){
            $model = D('RecruitmentView'); 
        }else{
            $where['recruitment.title'] = array('like',"%$key%");
            $where['member.username'] = array('like',"%$key%");
          //  $where['category.title'] = array('like',"%$key%");
            $where['_logic'] = 'or';
            $model = D('RecruitmentView')->where($where); 
        } 
...
//             $model->user_id = 1; 
    ...
    public function update($id)
    {
        ...
        if (IS_POST) {
            ...
            if (!$model->create()) {
                ...
                if ($model->save()) {
                    $this->success("更新成功", U('Recruitment/index'));
                } else {
                    $this->error("更新失败");
                }        
            }
    ...
    public function delete($id)
    {
        ...
        if($result){
            $this->success("删除成功", U('Recruitment/index'));
        }else{
            $this->error("删除失败");
        }
    }
