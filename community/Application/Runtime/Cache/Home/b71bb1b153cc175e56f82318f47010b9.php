	...
	<div class="container" style="margin-top:0px;min-height:550px;background-color:white;">
		<div class="row">
			<div class="col-md-3" style="margin-top:20px;margin-left:0px;margin-right:-60px;"> 
				...
				<div style="background-color:#c0153f;height:90px;line-height:90px;color:#ffffff;font-family:黑体;font-size:23px;padding-left:10px;">高校合作项目</div>
				<div style="background-color:#b95253;height:40px;line-height:40px;color:#ffffff;font-family:黑体;font-size:16px;padding-left:10px;">UNIVERSITY COOPERATING PROGRAM</div>
			...
			<div class="col-md-8">
			   <h5 style="margin-top:20px;"><strong>高校合作项目</strong><small><span class="float_right"><a href="<?php echo U('Index/index');?>">首页</a> > 合作项目 > <a href="<?php echo U('ProgramUniversityCooperation/index');?>">高校合作项目</a></span></small></h5>
				<hr></hr>         
						<div role="tabpanel" class="tab-pane active" id="college-1140">
							<!-- name指定的名字就是从model层拿过来的数据对象，item表示该对象的每一条信息  -->
							<?php if(is_array($model)): foreach($model as $key=>$v): ?><h5>
								   <!-- 取title字段 -->
								   <a alt=""  href="<?php echo U('ProgramUniversityCooperation/contentdisplay?id='); echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></a>
								   <!-- 取time字段的值 -->
								   <small><span class="float_right"><?php echo (date("Y/m/d",$v["time"])); ?></span></small>
								</h5><?php endforeach; endif; ?>
						</div>     
						<div class="clearfix" style="margin: 0 auto;"> <?php echo ($page); ?>  </div>                                                                                                                          
			</div>
		</div>
	</div>
