...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-md-6">
            <a href="<?php echo U('Images/add');?>" class="btn btn-success">添加图片</a>
        </div>
        <div class="col-md-6">
            <form action="<?php echo U('Images/index');?>" method="post">
                <div class="form-group input-group">
                    <input type="text" class="form-control" name="key" placeholder="输入图片名称或模块名称关键词搜索">
    ...
    <table class="table table-hover table-striped table-bordered">
        ...
        <?php if(is_array($model)): foreach($model as $key=>$v): ?><tr>
				...
				<td>
                    <?php if($v["status"] == 0): ?><span class="label label-success">禁止</span>
                    <?php elseif($v["status"] == 1): ?><span class="label label-info">启用</span><?php endif; ?>
                </td>
                <td><?php echo (date("Y/m/d H:i:s",$v["time"])); ?></td>                            
                <td><?php echo ($v["username"]); ?></td>
                <td><a href="<?php echo U('Images/update?id='); echo ($v["id"]); ?>" >查看</a> | <a href="<?php echo U('Images/delete?id='); echo ($v["id"]); ?>" style="color:red;" onclick="javascript:return del('您真的确定要删除吗？\n\n删除后将不能恢复!');">删除</a></td>             
