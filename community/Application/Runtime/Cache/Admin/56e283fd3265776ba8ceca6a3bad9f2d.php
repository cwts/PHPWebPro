...
<div id="page-wrapper">
	 <div class="form-group">  		
    		<button class="btn btn-primary" id="selsect">请上传照片</button>
    ...
    <form method="post" action="<?php echo U('Workhouse/add');?>">
    	<div class="form-group">
        ...
        <div class="form-group">
            <label for="Workhouse-name">社工姓名</label>
            <input type="text" name="name" class="form-control" id="name" placeholder="输入社工姓名">
        </div>
   		 <div class="form-group">
            <label for="Workhouse-duty">职务职称</label>
            <input type="text" name="duty" class="form-control" id="duty" placeholder="输入职务职称">
        </div>
        <div class="form-group">
            <label for="Workhouse-major">专业方向</label>
            <input type="text" name="major" class="form-control" id="major" placeholder="输入专业方向">
        </div>
		<div class="form-group">
            <label for="Workhouse-qualification">专业资质</label>
            <input type="text" name="qualification" class="form-control" id="qualification" placeholder="输入专业资质">
        </div>
		<div class="form-group">
            <label for="Workhouse-year">从业年限</label>
            <input type="text" name="year" class="form-control" id="year" placeholder="输入从业年限">
        </div>       	  
...
<script type="text/javascript">
			...
			function html5up(file){
				...
				$.ajax({
					url: '<?php echo U("Workhouse/upload",null,false);?>',
					type: 'POST',
				...
				.done(function(data) {
                    ...
                    //alert($('#photoname').val());
					var photo="/community"+data.savepath+data.savename;
		            $('#photo').val(photo);
					//alert($('#photo').val());
					console.log("success");
				})
			...
			$(document).on('click','.save',function(event){
				...
				// alert(img_url);
				$.ajax({
					url: '<?php echo U("Workhouse/changeName",null,false);?>',
					type: 'POST',
					// dataType: 'default: Intelligent Guess (Other values: xml, json, script, or html)',
...
//						alert(data);return false;
		                ...
		                $('#photoname').val(photoname);
					    alert($('#photoname').val());
				     	var photo="/community"+data.savepath;
		                $('#photo').val(photo);
				     	alert($('#photo').val());
					}
				});
			...
			$(document).on('click', '.glyphicon-trash', function(event) {
			    ...
			    //alert(img_url);
				$.ajax({
					url: '<?php echo U("Workhouse/del",null,false);?>',
					type: 'POST',
					// dataType: 'default: Intelligent Guess (Other values: xml, json, script, or html)',
					data: {url: img_url},
					success:function(data){
